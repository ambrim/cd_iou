...
def get_voxel_coords(volume, threshold):
    coords = np.argwhere(volume >= threshold)
    grid_size = 128
    voxel_size = 4.5
    return (coords - np.array([grid_size // 2]*3)) * voxel_size

def chamfer_distance(gt_vol, pred_vol, pred_thresh, mask=None):
    gt_vol = apply_mask(gt_vol, mask)
    pred_vol = apply_mask(pred_vol, mask)
    gt_coords = get_voxel_coords(gt_vol, 0.05)
    pred_coords = get_voxel_coords(pred_vol, pred_thresh)
...
def compute_metrics(gt_path, pred_path, thresholds, mask_path=None):
    gt_vol = load_mrc(gt_path)
    pred_vol = load_mrc(pred_path)
    mask = load_mrc(mask_path) if mask_path else None
    results = []
    for t in thresholds:
        cd = chamfer_distance(gt_vol, pred_vol, t, mask)
        iou = volumetric_iou(gt_vol, pred_vol, t, mask)
        results.append((t, cd, iou))
    return results

def gather_all_metrics(gt_dir, pred_dir, thresholds, mask_path, output_csv):
    data = []
    ...
    for pred_file in pred_files:
        ...
        if not os.path.exists(gt_file):
            ...
            continue
        metrics = compute_metrics(gt_file, pred_file, thresholds, mask_path)
        for t, cd, iou in metrics:
            data.append({"Volume": basename, "Threshold": t, "Chamfer Distance": cd, "IoU": iou})
...
def plot_metrics(df, dataset_name, output_dir):
    models = df['Model'].unique() if 'Model' in df else ['Unnamed']
    colors = ['#da70d6', '#75E6DA', '#a569bd', '#2E5984'][:len(models)]
    fig, axs = plt.subplots(1, 2, figsize=(12, 5))

    for i, model in enumerate(models):
        subdf = df[df['Model'] == model] if 'Model' in df else df
